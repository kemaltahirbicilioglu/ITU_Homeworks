...
int get_inputs(fstream &data_fptr, Employee *employees){ // it returns the number of employees added to the array.
    ...
    getline(data_fptr, line); // discard the first line of the data file.
...
int clear_empty_spaces(Employee *employees, Employee *new_employees, int employee_size){ // it returns the last id of the new_employees array.
    ...
    int last_id = new_employees[employee_size - 1].get_id(); 
    return last_id;    
}
...
//clock_t start = clock();
    ...
    for(int i = 0; i < employee_size; i++){
        output_file << employee_ptr[i].get_id() << ";" << employee_ptr[i].get_salary() << ";" << employee_ptr[i].get_department() << "\n";
    }
    delete[] employee_ptr;
    output_file.close();
