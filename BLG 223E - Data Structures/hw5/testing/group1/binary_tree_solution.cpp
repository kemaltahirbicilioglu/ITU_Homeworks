...
void BinaryTree::insert(Employee* employee){
    ...
    while(temp != nullptr){
        ...
        if(id < curr_id){
            ...
            cout << "id: " << id << " will the bt have duplicate id's ??\n";
            return;
        }
    }
}
...
void BinaryTree::clear_content(Node* root){
    if(root == nullptr) {
        return;
    }
    ...
    while (current || !nodeStack.empty()) {
        if (current) {
            ...
            // Check if the right subtree needs to be traversed
            if (peekNode->get_right() && lastVisited != peekNode->get_right()) {
                current = peekNode->get_right();
            }else{
...
clock_t start = clock(); 
    ...
    while(getline(operation_fptr, line)){
        ...
        while(line[index] != ';' && !p_h){ // checks until it sees a ';' or it finds the operations PRINT or HEIGHT
            if(is_equal_strings("PRINT", operation)){
                p_h = true;
                cout << "P\n";
                bt.printToConsole(bt.get_root());
            }else if(is_equal_strings("HEIGHT", operation)){
                p_h = true;
                int height = 0;
                bt.getHeight(bt.get_root(), &height, 0);
                cout << "H " << height << "\n";
            } 
        ...
        if(is_equal_strings("ADD", operation)){
            ...
            if(bt.get_root() == nullptr){
                ...
                Employee* check_employee = bt.search(id);
                if(check_employee->get_id() != id){
                    cout << "ERROR: An invalid ID to delete\n";
                    delete check_employee;
                }else{
                    bt.set_root(bt.remove(id)); 
                }
            }
    ...
    Node* temp = bt.get_root();
    bt.printToFile(bt_solution_fptr, temp);
