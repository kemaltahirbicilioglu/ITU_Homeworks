...
int find_last_id(fstream &input_file, fstream &output_file, int *file_size){
    input_file.clear();
    input_file.seekg(0, ios::beg); // make sure it points to the beginning of the file
    ...
    int last_id = 0;
    int count = 0;
    getline(input_file, line); // discard the first line.
    output_file << line << "\n";
    ...
    *file_size = count;
    return last_id; // last taken id is the last id of the employees file.
}
...
int main(int argc, char *argv[]){
    ...
    // argc argv...
    string data_file, operation_file, output_file;
    output_file = "file_solution.csv";
    if(argc == 3){
        data_file = argv[1];
    ...
    fstream data_fptr, operation_fptr, output_fptr;

    data_fptr.open(data_file, ios::in);
    operation_fptr.open(operation_file, ios::in);
    output_fptr.open(output_file, ios::out);

    int file_size = 0;  // to print out there is no employee in the file if there is not.
    int last_id = find_last_id(data_fptr, output_fptr, &file_size);
...
clock_t start = clock();
    ...
    while(getline(operation_fptr, line)){
        ...
        if(is_equal_strings("ADD", operation)){
            ...
            output_fptr.open(output_file, ios::app);
            ...
            last_id++;
            file_size++;
            add_employe_to_file(output_fptr, last_id, salary, department);
            output_fptr.close();
            ...
            int department = stoi(s_department);
            // we got datas.
            output_fptr.open(output_file, ios::in);
            bool is_id_valid = valid_id_check(output_fptr, id);
            if(!is_id_valid){
            ...
            update_employee(output_fptr, temp_fptr, id, salary, department);
            temp_fptr.close();
            output_fptr.close();
            
            output_fptr.open(output_file, ios::out); // to get rid of old file. when opening with an out mode, it will automatically delete the old datas.
            temp_fptr.open(temp_file, ios::in);

            copy_back_files(temp_fptr, data_fptr);
            temp_fptr.close();
            output_fptr.close();
            remove(temp_file.c_str()); // delete the file.
            ...
            output_fptr.open(output_file, ios::in);
            bool is_valid_id = valid_id_check(output_fptr, id);
            if(!is_valid_id){
            ...
            delete_employee_from_file(output_fptr, temp_fptr, id);
            temp_fptr.close();
            output_fptr.close();
            ...
            copy_back_files(temp_fptr, output_fptr);
            temp_fptr.close();
            output_fptr.close();
